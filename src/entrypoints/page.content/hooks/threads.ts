import type { Threads } from '@midra/nco-api/types/niconico/threads'
...
export const hookThreads = async (
  ...
  try {
    ...
    if (json.meta.status !== 200 || !json.data) {
      return new Response(JSON.stringify(json), resInit)
    }

    let { globalComments, threads } = json.data
