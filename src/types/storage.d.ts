...
export interface SettingItems {
   ...
   * @default true
   */
  'comment:showExtra': boolean

  /**
   * 設定: コメント > 引用コメントを統合
   * @default true
   */
  'comment:mergeExtra': boolean

  /**
   * 設定: コメント > 引用コメントを半透明化
   * @default true
   */
  'comment:translucentExtra': boolean

  /**
   * 設定: コメント > 引用コメントの色
   * @default '#FFFFFF'
   */
  'comment:extraColor': string

  /**
   * 設定: コメント > 引用コメントの色を強制
   * @default false
   */
  'comment:forceExtraColor': boolean

  /**
   * 設定: コメント > かんたんコメントを表示
   * @default true
   */
  'comment:showEasy': boolean
   ...
   * @default 'アニメ'
   */
  'searchOptions:genre': '未指定' | NiconicoGenre

  /**
   * 検索オプション: 再生時間
   * @default [null, null]
   */
  'searchOptions:lengthRange': [start: number | null, end: number | null]
}
...
export type StorageItems = InternalItems & SettingStorageItems & SlotItems
